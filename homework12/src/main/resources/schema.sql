...
ALTER TABLE AuthorsBooks ADD FOREIGN KEY (idBook) REFERENCES Books(idBook) ON DELETE CASCADE;
ALTER TABLE AuthorsBooks ADD FOREIGN KEY (idAuthor) REFERENCES Authors(idAuthor) ON DELETE CASCADE;

CREATE TABLE GenresBooks(idBook BIGINT, idGenre BIGINT);
ALTER TABLE GenresBooks ADD FOREIGN KEY (idBook) REFERENCES Books(idBook) ON DELETE CASCADE;
ALTER TABLE GenresBooks ADD FOREIGN KEY (idGenre) REFERENCES Genres(idGenre) ON DELETE CASCADE;

CREATE TABLE Users(id UUID, username VARCHAR(255), password VARCHAR(255));